...
export default function CreateTokengate() {
  ...
  const { fields, submit, submitting, dirty, reset, makeClean } = useForm({
    ...
    onSubmit: async (formData) => {
      const { discountType, discount, name, products, orderAmount } = formData;
      ...
      if (response.ok) {
        setToastProps({ content: "Loyality Program Created" });
        makeClean();
        navigate("/");
      } else {
        setToastProps({
          content: "There was an error creating a program",
          error: true,
        });
      }
    },
  ...
  return (
    <Page
      ...
      title="Create a new Loyality Tokens Program"
    >
      <Layout>
        <Layout.Section>
            ...
            <Layout>
              <Layout.Section>
                <Card>
                  ...
                  <Card.Section title="Minumum Order Amount">
                    <TextField
